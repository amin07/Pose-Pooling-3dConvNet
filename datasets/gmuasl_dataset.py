...
def load_rgb_frames_from_video(vid_root, vid, start, num, resize=(256, 256)):
    ...
    video_path = os.path.join(vid_root, vid)      # already .avi
...
def make_dataset(subs, split, root, mode, num_classes):
    dataset = []
    count_skipping = 0 
    for vid_file in glob.glob(root+'/*'):
      video_path = vid_file
      vid = os.path.basename(vid_file)
      sub = vid.split('-')[1].split('_')[1]
      ...
      """
      # this is done in get item
      label = np.zeros((num_classes, num_frames), np.float32)
      for l in range(num_frames):
        class_name = os.path.basename(vid_file).split('-')[1].split('_')[0]
        c_ = self.class2label[class_name]
        label[c_][l] = 1
      """
      label = vid.split('-')[1].split('_')[0]
      dataset.append((vid, label, 0, 0, num_frames))
    print ('dataset size', len(dataset), 'skipped', count_skipping, 'subs', subs)
    return dataset
    sys.exit()
...
class GMUASLDataset(data_utl.Dataset):

    def __init__(self, subs, split, root, mode, transforms=None):
        """ subs : cross subject, which subjects data """
        #self.num_classes = get_num_class(split_file)
        self.num_classes = self._create_class_dicts(subs, root)
        self.data = make_dataset(subs, split, root, mode, num_classes=self.num_classes)
    ...
    def _create_class_dicts(self, subs, root):
      classes = sorted(list(set([f.split('-')[1].split('_')[0] for f in os.listdir(root)])))
      self.class2label = dict([(c, i) for i, c in enumerate(classes)])
      self.label2class = dict([(i, c) for i, c in enumerate(classes)])
      return len(classes)
    ...
    def __getitem__(self, index):
        ...
        total_frames = num_frames = nf
        start_f = 0
        if self.split=='test':      # this is doing the nstl all version
        ...
        for l in range(num_frames):
          class_name = vid.split('-')[1].split('_')[0]
          c_ = self.class2label[class_name]
          label[c_][l] = 1 
